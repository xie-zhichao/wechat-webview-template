...
// const baseUrl = 'http://192.167.5.111:7041';
const baseUrl ="http://supe.nongqibang.com:7041"
...
function tranData(data, path = '') {
  const str = Object.keys(data).map((e) => {
    control.add(`${e.replace(/-/g, '').replace(/\s+/g, "")  }_${  path.split('/').join('').toLocaleUpperCase()}`);
    return tranParent(`${e.replace(/-/g, '').replace(/\s+/g, "")  }_${  path.split('/').join('').toLocaleUpperCase()}`, data[e], path);
    // if (control.has(e)) {
      ...
      axios.get(baseUrl + urlAddress)
        .then((e) => {
          if (e.status === 200) {
            try {
              const obj = {};
              const path = e.data.basePath.split('/')[1];
...
//  axios.get("http://192.167.5.111:7041/swagger-resources")
 axios.get("http://supe.nongqibang.com:7041/swagger-resources")
    .then(async (e) => {
      let filterResult = e.data.filter(ed => (filterData.length > 0 ? filterData.some((ef) => ed.url.indexOf(ef) !== -1) : true));
      filterResult = filterResult.filter((ed) => ed.url.indexOf('sso') == -1 && ed.url.indexOf('app') == -1);
    ...
    `;
      fs.writeFileSync('./command/api.js', text, 'utf8');
    });
}
